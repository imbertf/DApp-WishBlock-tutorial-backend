...
describe("WishList", function () {
  ...
  describe('buyItem', () => {
    ...
    it('should NOT by item if not enough funds are provided', async function () {
      const etherGiven = ethers.parseEther('1');
      await expect(wishList.connect(addr1).buyItem(owner.address, 1, { value: etherGiven })).to.be.revertedWithCustomError(wishList, "NotEnoughFunds")
    })
    ...
    it('should NOT buy the item if the item has already bought', async function () {
      const etherGiven = ethers.parseEther('3');
      await wishList.connect(addr1).buyItem(owner.address, 1, { value: etherGiven })
      await expect(wishList.connect(addr2).buyItem(owner.address, 1, { value: etherGiven })).to.be.revertedWithCustomError(wishList, "AlreadyBought")
    })
  })
});
